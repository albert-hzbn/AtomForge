...
CMake checks that this PDF exists before configuring the application. Development builds outside the source tree retain its source location as a fallback; installed and portable manual locations take precedence. The cover logo is the existing 256-pixel application icon from `assets/icon/atomforge.ico`, exported as `figures/atomforge-logo.png` for pdfLaTeX.
