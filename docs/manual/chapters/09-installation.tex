...
The source uses C++17 and CMake 3.16 or newer. Desktop dependencies include GLFW, GLEW, GLM, Eigen3, Open Babel 3, and an OpenGL implementation. spglib enables symmetry-related capabilities. Use a consistent compiler/runtime family for all linked libraries. See the repository's \code{INSTALL.md} and actual CMake configuration for platform-specific dependency discovery.
...
\section{Linux build}
Install a C++ toolchain, CMake, pkg-config, and development packages for GLFW, GLEW, GLM, Eigen3, Open Babel, and optionally spglib using your distribution's package manager. Configure a fresh build directory:
\begin{lstlisting}
...
Use the MSYS2 UCRT64 toolchain and its matching dependency packages. Typical packages include the UCRT64 toolchain, CMake, GLFW, GLEW, GLM, Eigen3, Open Babel, pkgconf, spglib, and Python when running the Python tests. Avoid mixing MINGW64, UCRT64, and unrelated compiler libraries in one build directory.
