...
\code{AtomForge --build MODE --help} prints the mode's accepted options. Available documented modes are bulk, sss, gb, nano, poly, amorphous, and custom. Each returns an output structure without requiring a GUI interaction. Use a subprocess argument list from Python to preserve spaces and avoid shell quoting problems:
\begin{lstlisting}[language=Python]
import os
import subprocess
subprocess.run([
