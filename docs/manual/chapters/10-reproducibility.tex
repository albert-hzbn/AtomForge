...
\section{Nanocrystal shape examples}
The seven shape-cut examples in \code{examples/shapes/} supplement the inventory above. \code{shape-results.json} records the native command options and atom counts. Chapter~\ref{ch:tutorials} shows each shape and provides an exercise for rebuilding it. The example checker verifies all seven files for count, Cu species, and finite coordinates.
...
The illustrated exercises are maintained in \code{tutorials.json}. After changing their steps, captions, or image crops, run \code{python docs/manual/scripts/render_tutorials.py --write}. It validates image availability, bounds, tutorial identifiers, and required exercise fields before regenerating the chapter. Normal builds check that the generated chapter is current. Original Windows screenshots are retained in \code{figures/tutorials/}; crops are applied in LaTeX, without redrawing the interface. This collection also retains alternative option panels for reuse.
...
\end{lstlisting}
The script runs three passes for contents, references, and bookmarks. It writes \code{docs/manual/AtomForge-manual.pdf}; intermediates go into the ignored \code{docs/manual/build/} folder. Published figures are included, so compilation requires no original CHGCAR.
...
\bottomrule
\end{longtable}
Update the version and edition date when revising the manual. Re-run examples after algorithm or format changes, compare the generated metadata, and recapture affected UI figures. Avoid describing a feature as available solely because a corresponding source file exists.
