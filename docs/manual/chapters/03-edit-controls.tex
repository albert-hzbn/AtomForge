...
\item[Use anisotropic elasticity.] Replaces the isotropic Poisson-ratio displacement field with the Stroh sextic formalism for a fully anisotropic elastic medium, given the crystal's elastic constants ($C_{11}$, $C_{12}$, $C_{44}$, and $C_{13}$/$C_{33}$ for hexagonal symmetry). A small noise amplitude breaks the sextic solve's degeneracy at isotropic or high-symmetry line/plane orientations.
\item[Insert as dipole.] Superposes a second dislocation of opposite Burgers vector at a specified offset, giving a net-zero-Burgers-vector configuration that is periodicity-compatible, unlike a single dislocation in a periodic cell.
\end{description}
This tool and its post-processing counterpart (Section~\ref{sec:dislocation-postprocessing}) are native reimplementations of the physical models and algorithms used by the BABEL dislocation toolkit (Douin, Clouet et al.); see that section for the full literature list.
