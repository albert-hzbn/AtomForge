...
{\centering
\makebox[\linewidth][c]{%
  \llap{\raisebox{-.25\height}{\includegraphics[height=14mm]{figures/atomforge-logo.png}}\hspace{5mm}}%
...
\vspace{14mm}
\input{chapters/00-cover-gallery}
\vfill
{\large Version 0.3.0\par}
{\large Documentation edition: 11 September 2026\par}
\end{titlepage}
\pagenumbering{roman}
...
Menu names and controls describe the documented application features and the Windows executable used to prepare the figures. Features controlled by build options are marked explicitly. A source file does not by itself mean a feature is accessible in the desktop menu. Common-neighbour analysis, angular-distribution analysis, the interface builder, and stacking-fault generation are enabled in the current source build. The scientific integrations and project workflows are described in Chapter~\ref{ch:workflows}; these additions require a rebuilt application and the matching Python source package. The detailed builder, edit, and display control references supplement the worked examples. Appendix~\ref{ch:coverage} maps the feature families back to source and explains the documentation coverage check.

GUI screenshots, including all six cover views, are captured from AtomForge. The cover keeps the application's default atomic radii; camera framing varies between examples. The CHGCAR plots and shape-cut diagrams are separately rendered scientific figures calculated from real AtomForge outputs. Example structures are starting configurations, not validated equilibrium structures. The supplied CHGCAR is used to demonstrate the software; its physical system and calculation settings were not independently established.
...
\input{chapters/01-start}
\input{chapters/02-builders}
\input{chapters/02-builder-controls}
\input{chapters/03-editing}
\input{chapters/03-edit-controls}
\input{chapters/04-analysis}
\input{chapters/04-display-controls}
\input{chapters/05-electronic}
\input{chapters/06-charge}
\input{chapters/07-reference}
\input{chapters/08-python}
\input{chapters/14-workflows}
\input{chapters/13-illustrated-tutorials}
\appendix
\input{chapters/09-installation}
\input{chapters/10-reproducibility}
\input{chapters/11-coverage}
\input{chapters/12-api-signatures}
\end{document}
